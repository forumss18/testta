...
if st.sidebar.checkbox("Load Existing Dataset", value = False,  help=readme["tooltips"]["existing_upload"]) == True: 
    filename = st.sidebar.selectbox('Choose a CSV files',  
    ...
    if filename:
        ...
        data = data[['Timestamp', 'Longitude', 'Latitude', 'Speed', 'Operator', 'CellID', 'LAC', 'LTERSSI', 'RSRP', 'RSRQ', 'SNR', 'DL_bitrate', 'UL_bitrate']]        
else:
    ...
    if file is not None:
        ...
        if file.type == 'text/csv' or file.type == 'application/vnd.ms-excel':
            ...
            data = data.rename(columns={
                        ...
                        'Qual' : 'RSRQ'})
            data = data[['Timestamp', 'Longitude', 'Latitude', 'Speed', 'Operator', 'CellID', 'LAC', 'LTERSSI', 'RSRP', 'RSRQ', 'SNR', 'DL_bitrate', 'UL_bitrate']]
            data['Timestamp'] = data['Timestamp'].apply(lambda x: x.split('_')[0].replace('.', '-') + ' ' + x.split('_')[1].replace('.', ':'))
...
if data is not None:
    ...
    if heatmap == True:
        ...
        st.pyplot(figure)
        with st.expander("About Correlation Heatmap", expanded=False):
    ...
    features_names = st.sidebar.multiselect("Choose a Feature", list(data.columns), ['Timestamp'], help=readme["tooltips"]["select_feature"]) 
                                    
    target = 'DL_bitrate'
    features = data[features_names] # dataframe dengan column yang dipilih sebagai feature
    ...
    if st.sidebar.checkbox("PCA (Optional)", help=readme["tooltips"]["cekbox_pca"]):
        pca_features = st.sidebar.multiselect("Pilih fitur mana yang akan diproses kedalam PCA", features_names, help=readme["tooltips"]["select_pca"]) # pilih variabel/column2 yang akan digabungkan menggunakan pca
    ...
    st.sidebar.write("Bagian ini digunakan untuk proses modelling dan evaluation data")
    ...
    if model == 'k-Nearest Neighbor':
        ...
        knn.fit(train_features.drop('Timestamp', axis=1), train_target)

        pred_target = knn.predict(test_features.drop('Timestamp', axis=1))
        ...
        n_estimators = st.sidebar.slider('Number Estimator', 1, 1000, value=5, help=readme["tooltips"]["random_forest"])

        RF = RandomForestRegressor(n_estimators=n_estimators)
        RF.fit(train_features.drop('Timestamp', axis=1), train_target)
        ...
        n_estimators = st.sidebar.slider('Number Estimator', 1, 1000, value=5, help=readme["tooltips"]["ada_boost"])

        boosting = AdaBoostRegressor(n_estimators=n_estimators)
        boosting.fit(train_features.drop('Timestamp', axis=1), train_target)
